...
getObsExp <- function(simulatedRDS, delta, a, f, k){
    ...
    mate.cor[[1]] <- cor(as.data.frame(simulatedRDS[[4]]))[1:2,3:4]
    rmate.t0 <- mate.cor[[1]]
    covE.t0 <- target$covE
    (exp.mu[[1]] <- solve(exp.VY[[1]]) %*% rmate.t0 %*% solve(t(exp.VY[[1]])))
    exp.w[[1]] <- target$w
    exp.v[[1]] <- target$q
    exp.Omega[[1]] <- target$omega
    exp.Gamma[[1]] <- target$gamma
     (mate.cov[[1]] <- exp.VY[[1]] %*% exp.mu[[1]] %*% exp.VY[[1]] )
    ...
    for (it in 2:3){
            ...
            (exp.Omega[[it]] <- 2*delta.t0%*%exp.gc[[it]] + delta.t0 %*% k.t0 + .5*exp.w[[it-1]] + 2*a.t0%*%exp.ic[[it]])
            cat("Omega",it,"\n",exp.Omega[[it]],"\n")
            (exp.Gamma[[it]] <- 2*a.t0%*%exp.hc[[it-1]] + 2*delta.t0%*%t(exp.ic[[it]]) + a.t0%*%j.t0 + .5*exp.v[[it-1]])
            cat("Gamma",it,"\n",exp.Gamma[[it]],"\n")
        ...
        EXP <- list(
            ...
            gc=exp.gc[[gens-1]],
            hc=exp.hc[[gens-1]],
            itlo=exp.itlo[[gens-1]],
            itol=exp.itol[[gens-1]],
            ic=exp.ic[[gens-1]],
            VF=exp.VF[[gens-1]],
            w=exp.w[[gens-1]],
            q=exp.v[[gens-1]],
