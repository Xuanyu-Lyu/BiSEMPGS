# a script to test if the interaction between r2pgs and v12 estimation precision can be reproduced in other settings
...
sample_size_names <- c("4k", "8k", "16k", "32k", "48k", "64k")
...
startingParamList1 <- list(vg1 = rep(.64,5),
						   ...
						   am21 = c(.1,.1,0.1,.1,.1), 
						   am22 = c(rep(0.3,4),.3),
						   f11 = c(rep(0.15,4),0.15),
						   f12 = c(0.1,.1,.1,.1,.1),
						   f21 = c(0.05,.05,.05,.05,.05), 
						   f22 = rep(0.1,5),
						   Nfam = rep(8e4, 5),
						   rg = rep(0.1,5),
						   re = rep(0.1,5),
						   prop.h2.latent1 = c(.63/.64,.62/.64, .60/.64, .56/.64, .48/.64), 
						   prop.h2.latent2 = rep(.8,5))

for (condition in 3){
    # Loop through each sample size
    for (ss_idx in 2){
        num_samples <- sample_sizes[ss_idx]
        ss_name <- sample_size_names[ss_idx]
        
        cat(paste0("\n=== Running condition ", condition, " with sample size ", ss_name, " (", num_samples, ") ===\n"))
	...
	for (it in 2:gens){
			...
			(exp.ic[[it]] <- .5*(exp.itlo[[it]] +t(exp.itol[[it]])))
			#cat("ic",it,"\n",exp.ic[[it]],"\n")
	...
	# Generate samples for this sample size
	for (iter in c(1:69, 80:500)){
	    set.seed(123+iter) 
	    samples <- mvrnorm(n = num_samples, mu = rep(0, nrow(CMatrix)), Sigma = CMatrix, empirical = FALSE)
	    # Save each sample to a file as a tsv
	    sample_file <- paste0("Data/Paper/test_v12_ss/", conditionNames[condition], "_", ss_name, "/samples_iter_empi_", iter, ".tsv")
	...
	for (i in c(1:339, 350:500)){
	    DataPath <- paste0("Data/Paper/test_v12_ss/", conditionNames[condition], "_", ss_name, "/samples_iter_empi_", i, ".tsv")
		#testData <- read.table(DataPath, header = TRUE, sep = "\t")
        # fit the model
        fit <- fitBiSEMPGS_m2_tol(DataPath,
                                      feaTol = 1e-6, 
                                      optTol = 1e-9,
                                      jitterMean = 0.5,
                                      jitterVar = .1,
        ...
        fit_fixedA <- fitBiSEMPGS_m2_tol_fixH2(DataPath,  avalue = c(a11, a22),
                                      feaTol = 1e-6, 
                                      optTol = 1e-9,
                                      jitterMean = 0.5,
                                      jitterVar = .1,
                                      exhaustive = FALSE,
                                      extraTries = 5)
        summary_list_fixedA[[paste0("iter_", i)]] <- fit_fixedA
        cat("\nMVN samples iteration", i, "has been fitted with fixed A\n")
	}

	# save the list of summaries
	save_path <- paste0("Analysis/Paper/test_v12_ss/", conditionNames[condition], "_", ss_name, "_MVN_summary_list_500.rds")
	dir.create(dirname(save_path), recursive = TRUE, showWarnings = FALSE)
	saveRDS(summary_list, save_path)
    save_path_fixedA <- paste0("Analysis/Paper/test_v12_ss/", conditionNames[condition], "_", ss_name, "_MVN_summary_list_fixedA_500.rds")
    saveRDS(summary_list_fixedA, save_path_fixedA)
    
    cat(paste0("\nCompleted sample size ", ss_name, " for condition ", condition, "\n"))
