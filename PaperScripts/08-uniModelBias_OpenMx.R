...
fitUniSEMPGS_diffDelta <- function(data, max.cores = 2){
    library(OpenMx)
    library(data.table)
    library(stringr)
...
#   options()$mxOptions$'Optimality tolerance'  #1e-7
  ...
  f <- mxMatrix(type="Lower",nrow=nv,ncol=nv,free=TRUE,values=0.4,label="F",name="f",lbound=-.05)
  e <- mxMatrix(type="Lower",nrow=nv,ncol=nv,free=TRUE,values=.3,label="EnvPath",name="e",lbound=-.05)
  e_last <- mxMatrix(type="Lower",nrow=nv,ncol=nv,free=TRUE,values=.3,label="EnvPath_last",name="e_last",lbound=-.05)
  g <- mxMatrix(type="Lower",nrow=nv,ncol=nv,free=TRUE,values=.01,label="hap_PRS_cov",name="g")
  h <- mxMatrix(type="Lower",nrow=nv,ncol=nv,free=TRUE,values=.01,label="latent_hap_PRS_cov",name="h")
  delta <- mxMatrix(type="Lower",nrow=nv,ncol=nv,free=TRUE,values=.3,label="obs_coef",name="delta",lbound=-.05)
  delta_last <- mxMatrix(type="Lower",nrow=nv,ncol=nv,free=TRUE,values=.3,label="obs_coef_last",name="delta_last",lbound=-.05)
  a <- mxMatrix(type="Lower",nrow=nv,ncol=nv,free=FALSE,values=0.7745967,label="latent_coef",name="a",lbound=-.05)
  a_last <- mxMatrix(type="Lower",nrow=nv,ncol=nv,free=TRUE,values=0.5377226,label="latent_coef_last",name="a_last",lbound=-.05)
  ...
  i1 <- mxMatrix(type="Lower",nrow=nv,ncol=nv,free=TRUE,values=0.02,label="covOL",name="i1")
  sigma1 <- mxMatrix(type="Lower",nrow=nv,ncol=nv,free=TRUE,values=1.5,label="sigma",name="sigma1")
  sigma1_last <- mxMatrix(type="Lower",nrow=nv,ncol=nv,free=TRUE,values=2.8698354,label="sigma_last",name="sigma1_last")
  Omega1 <- mxMatrix(type="Lower",nrow=nv,ncol=nv,free=TRUE,values=0.3,label="Omega",name="Omega1")
  Gamma1 <- mxMatrix(type="Lower",nrow=nv,ncol=nv,free=TRUE,values=0.2, label="Gamma",name="Gamma1")
  ...
  hCon <- mxConstraint(h==h2,name="hCon")
  sigmaCon <- mxConstraint(sigma1==sigma2,name="sigmaCon")
  sigmaCon_last <- mxConstraint(sigma1_last==sigma2_last,name="sigmaCon_last")
  OmegaCon <- mxConstraint(Omega1==Omega2,name="OmegaCon")
  GammaCon <- mxConstraint(Gamma1==Gamma2,name="GammaCon")


  # mxAlgebra for implied parameters and relative covariances
  sigma2 <- mxAlgebra(2*a*Gamma1 + 2*delta*Omega1 + a*v1 + delta*w1 + 2*f^2*sigma1*(1 + sigma1*mu1) + e_last^2,name="sigma2")
  sigma2_last <- mxAlgebra(2*delta_last^2*k + 4*delta_last^2*g + 2*a_last^2*j + 4*a_last^2*h + 8*delta_last*i1*a_last + 2*delta_last*w1 + 2*a_last*v1 + 2*f^2*sigma1*(1 + sigma1*mu1) + e^2,name="sigma2_last")
  ThetaNTM05 <- mxAlgebra(2*delta_last*g + 2*a_last*h + .5*w1,name="ThetaNTM05") #this is HALF theta_NTM in the math
  ThetaTM05 <- mxAlgebra(delta_last*k + ThetaNTM05,name="ThetaTM05")
  spsPRS <-mxAlgebra(sigma1*mu1*Omega1,name="spsPRS") #THIS HAS BEEN FIXED; more general: between parent Y and spouse's PRS
  PO <- mxAlgebra((a_last*Gamma1 + delta_last*Omega1 + f*sigma1)*(1+mu1*sigma1),name="PO")
  spouse <-mxAlgebra(mu1*sigma1^2,name="spouse")
  ...
  CVmat<-    mxAlgebra(rbind(
    #       NTm       Tm          NTp     Tp          Ym         Yp        Yo
    cbind(  k+g        ,g         ,g          ,g        ,Omega1     ,spsPRS    ,ThetaNTM05   ),    #NTm
    cbind(  g          ,k+g       ,g          ,g        ,Omega1     ,spsPRS    ,ThetaTM05        ),    #Tm
    cbind(  g          ,g         ,k+g        ,g        ,spsPRS     ,Omega1    ,ThetaNTM05   ),    #NTp
    cbind(  g          ,g         ,g          ,k+g      ,spsPRS     ,Omega1    ,ThetaTM05     ),    #Tp
    cbind(  Omega1     ,Omega1    ,spsPRS     ,spsPRS   ,sigma2     ,spouse    ,PO         ),    #Ym
    cbind(  spsPRS     ,spsPRS    ,Omega1     ,Omega1   ,spouse     ,sigma2    ,PO         ),    #Yp
    cbind(  ThetaNTM05 ,ThetaTM05    ,ThetaNTM05 ,ThetaTM05   ,PO         ,PO        ,sigma2_last     ) ),  #Yo
    dimnames=list(colnames(dat_SEM),colnames(dat_SEM)),name="expCov")
  ...
  params <- list(k ,j,  f , e , e_last, g , 
                 h, delta, delta_last, a, a_last,
                 #x1 , 
                 w1 ,mu1 , i1, sigma1, sigma1_last, Omega1 , Gamma1,
                 #x2,
                 w2, v1,v2, i2, g2,
                 h2,
                 Omega2 , Gamma2, sigma2, sigma2_last,
                 #xCon, 
                 wCon, 
                 iCon, 
                 gCon, 
  ...
  dat_SEM2 <- mxData(observed=dat_SEM, type="raw",  means=v_Mean)
  modelAFE <- mxModel("AFEmodel",params,dat_SEM2)
  print(mxCheckIdentification(modelAFE)) # Check model identification
  AFE.Fit=mxRun(modelAFE,intervals=TRUE,silent=FALSE)
  print("Model fitting done")
