fitBiSEMPGS_m2_tol_diffDelta <- function(data_path,feaTol = 1e-6, optTol = 1e-8, jitterMean = .5, jitterVar = .1, extraTries = 30, exhaustive = F){
        # load the packages
    library(OpenMx)
    library(data.table)
    library(stringr)
    ...
    # Create variables and define the algebra for each variables
        ...
        VY_O  <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(1.5,.5,.5,1.5), label=c("VY_O11", "VY_O12", "VY_O12","VY_O22"), name="VY_O", lbound = -.05) # Phenotypic variance due to other factors
        VE_O  <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.5,.06,0.06,.4), label=c("VE_O11", "VE_O12", "VE_O12","VE_O22"), name="VE_O", lbound = -.05) # Residual variance due to other factors
        ...
        VY_O_Algebra <- mxAlgebra(2*delta_O%*%k_O%*%t(delta_O) + 4*delta_O%*%gc%*%t(delta_O) + 2*a_O%*%j_O%*%t(a_O) + 4*a_O%*%hc%*%t(a_O) + 4*delta_O%*%t(ic)%*%t(a_O) + 4*a_O%*%ic%*%t(delta_O) +
                              delta_O%*%t(w) + w%*%t(delta_O) + a_O%*%t(v) + v%*%t(a_O) + VF_Algebra + VE_O, name="VY_O_Algebra")

        VY_Constraint    <- mxConstraint(VY == VY_Algebra,       name='VY_Constraint')
        VY_O_Constraint <- mxConstraint(VY_O == VY_O_Algebra, name='VY_O_Constraint')
    ...
    # Genetic effects:
        ...
        delta_O <- mxMatrix(type="Diag", nrow=2, ncol=2, free=c(T,T), values=c(.4,.3), label=c("delta_O11", "delta_O22"),name="delta_O", lbound = -.05) 
        ...
        a_O   <- mxMatrix(type="Diag", nrow=2, ncol=2, free=c(T,T), values=c(.8,.55), label=c("a_O11", "a_O22"),name="a_O", lbound = c(.1,.1)) 
        ...
        k_O   <- mxMatrix(type="Symm", nrow=2, ncol=2, free=matrix(c(F,T,T,F),nrow = 2,ncol = 2), values=c(.5,0.02,0.02,.5), label=c("k_O11", "k_O12", "k_O12","k_O22"),    name="k_O", lbound = -.05)     # PGS variance (if no AM)
        ...
        j_O   <- mxMatrix(type="Symm", nrow=2, ncol=2, free=matrix(c(F,T,T,F),nrow = 2,ncol = 2), values=c(.5,0.03,0.03,.5), label=c("j_O11", "j_O12", "j_O12","j_O22"),    name="j_O", lbound = -.05)     # Latent PGS variance (if no AM)
    ...
    # add a constraint to make k=j
        ...
        j_O_Algebra <- mxAlgebra(k_O, name = "j_O_Algebra")
        j_O_constraint <- mxConstraint(j_O == j_O_Algebra, name = "j_O_constraint")
    ...
    # Between-people covariances
        ...
        thetaT  <- mxAlgebra(1 * delta_O %*% k + thetaNT, name="thetaT")
        ...
        Yo_Yp   <- mxAlgebra(delta_O %*% t(Omega) + a_O %*% t(Gamma) + delta_O %*% t(Omega) %*% t(mu) %*% VY + a_O %*% t(Gamma) %*% t(mu) %*% VY + f %*% VY + f %*% VY %*% t(mu) %*% VY, name = "Yo_Yp")
        Yo_Ym   <- mxAlgebra(delta_O %*% t(Omega) + a_O %*% t(Gamma) + delta_O %*% t(Omega) %*% mu %*% VY + a_O %*% t(Gamma) %*% mu %*% VY + f %*% VY + f %*% VY %*% mu %*% VY, name = "Yo_Ym")
    ...
    # Expected covariances matrix
        CovMatrix <- mxAlgebra(rbind(
            ...
            cbind(Yo_Yp,     Yo_Ym,     VY_O_Algebra, thetaT,   thetaNT,      thetaT,  thetaNT), #Yo1 Yo2
            cbind(t(Omega),  t(Ym_PGSp),t(thetaT),  k_O+gc,    gc,           gt,      gt),      #Tp1 Tp2
            cbind(t(Omega),  t(Ym_PGSp),t(thetaNT), gc,        k+gc,         gt,      gt),      #NTp1 NTp2
            cbind(t(Yp_PGSm),t(Omega),  t(thetaT),  t(gt),     t(gt),        k_O+gc,  gc),      #Tm1 Tm2
    ...
    # Specify what parameters we're going to be including in our model:
        Params <- list(
                    VY, VY_O,
                    #VF, 
                    VE, VE_O, delta, delta_O, a, a_O, k, k_O, j, j_O, Omega, Gamma, mu, gt, ht, gc, hc, itlo, itol, ic, f, w, v,
                    VY_Algebra, 
                    VY_O_Algebra,
                    VF_Algebra,  
                    #VE_Algebra,
                    Omega_Algebra, Gamma_Algebra, adelta_Constraint_Algebra, j_Algebra, j_O_Algebra, gt_Algebra, ht_Algebra, gc_Algebra, hc_Algebra, gchc_constraint_Algebra, itlo_Algebra, itol_Algebra, ic_Algebra, w_Algebra, v_Algebra, wv_constraint_algebra,
                    VY_Constraint, 
                    VY_O_Constraint,
                    #VF_Constraint, 
                    #VE_Constraint,
    ...
    # Create the model:
        ...
        fitModel1 <- mxTryHard(Model1, extraTries = extraTries, OKstatuscodes = c(0,1), intervals=T, silent=F, showInits = F, exhaustive = exhaustive, jitterDistrib = "rnorm", loc=jitterMean, scale = jitterVar)
        #return(fitModel1)
