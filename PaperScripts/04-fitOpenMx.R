...
for (i in 3){
    for (j in 2:6){
        ...
        if (file.exists(paste0("Analysis/Paper/", conditionNames[i], "/m2_paper_version4", save_pattern[j], "_summary_list.rds"))){
            cat("Summary list for", conditionNames[i], "\t n=", folder_pattern[j], "has been fitted\n")
            next
        }
        summary_list <- list()
        for (k in c(1:250, 252:500)){
        ...
        save_path <- paste0("Analysis/Paper/", conditionNames[i], "/m2_paper_version4", save_pattern[j], "_summary_list.rds")
