...
# Update 04/16/25-Xuanyu
# The script try to make the last generation have different delta and a values to test the BiSEMPGS that allows parental and offspring delta and a to be different
...
reproduce <- function(MATES=MATES, XO=XO, XL=XL, PHEN=PHEN, CV.INFO=CV.INFO, CUR.GEN=CUR.GEN, cove.mat=cove.mat, fmat=f.mat, amat=a.mat, dmat=delta.mat, cor.list=am.list, covy.mat=COVY, k2.matrix=k2.matrix, last_gen = FALSE){
...
AM.SIMULATE <- function(CV.INFO, NUM.GENERATIONS, POP.SIZE, AVOID.INB, SAVE.EACH.GEN, SAVE.COVS, SEED, cove.mat, fmat, amat, dmat, cor.list, covy.mat, k2.matrix, last_gen = FALSE,
                        CV.INFO_last = NULL, dmat_last = NULL, amat_last = NULL, 
                        k2.matrix_last = NULL){
...
#Reproduce
#MATES=MATES,XO=XO,XL=XL,PHEN=PHEN,CV.INFO=CV.INFO,CUR.GEN=CUR.GEN,cove.mat=cove.mat; fmat=f.mat; amat=a.mat; dmat=delta.mat; cor.list=am.list; covy.mat=COVY; k2.matrix=k2.matrix)
if(CUR.GEN == NUM.GENERATIONS && last_gen){
    # If this is the last generation, we use the last generation's CV.INFO, dmat, and amat
    if(is.null(CV.INFO_last) || is.null(dmat_last) || is.null(amat_last)){
        stop("CV.INFO_last, dmat_last, and amat_last must be provided for the last generation.")
    }
    #CV.INFO <- CV.INFO_last
    dmat <- dmat_last
    amat <- amat_last
    k2.matrix <- k2.matrix_last
    cat("Using last generation's CV.INFO, dmat, and amat for reproduction.\n")
} 
OFFSPRING <- reproduce(MATES=MATES,XO=XO,XL=XL,PHEN=PHEN,CV.INFO=CV.INFO,CUR.GEN=CUR.GEN,cove.mat=cove.mat, fmat=fmat, amat=amat, dmat=dmat, cor.list=cor.list, covy.mat=covy.mat, k2.matrix=k2.matrix)
