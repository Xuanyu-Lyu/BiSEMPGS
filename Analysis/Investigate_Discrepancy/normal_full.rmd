...
startingParamList1 <- list(vg1 = c(.36,.36,.09,.36,.36,.18,.36),
                          ...
                          rg = rep(0,7),
                          re = rep(0,7),
                          prop.h2.latent1 = rep(0,7),
						  prop.h2.latent2 = rep(0,7))

    condition =7
    pathName = conditionNames[condition]
    # WILDCARD parameters
...
for(i in 1:50){
     l.summaryLast[[i]]<- readRDS(paste0("/Users/xuly4739/Library/CloudStorage/OneDrive-UCB-O365/Documents/coding/R-projects/BiSEMPGS/Summary/normal_full/loop", i, ".rds"))
}
...
l.empMateCor <- vector("list", 50)
l.empMateCov <- vector("list", 50)
l.empDelta <- vector("list", 50)
...
# read all the rds files into the list
for(i in 1:50){
    dataAll <- readRDS(paste0("/Users/xuly4739/Library/CloudStorage/OneDrive-UCB-O365/Documents/coding/R-projects/BiSEMPGS/Data/normal_full/loop", i, ".rds"))
    for (j in 2:16){
...
```{r comparison2}
  ...
  return(proportion_matrix)
    ...
    list(exp.w[[gens]], averages$w, element_wise_proportion(exp.w[[gens]], averages$w))
    list(exp.v[[gens]], averages$q, element_wise_proportion(exp.v[[gens]], averages$q))
    .5*(averages$covG + t(averages$covG))
