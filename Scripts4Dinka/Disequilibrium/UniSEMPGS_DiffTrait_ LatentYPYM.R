...
fitUniSEMPGS_DiffTrait_LatentYPYM <- function(data_path, h2_RDR_parent, h2_RDR_offspring, feaTol = 1e-6, optTol = 1e-8, jitterMean = .5, jitterVar = .1, extraTries = 30, exhaustive = F){
    library(OpenMx)
    library(data.table)
    library(stringr)
    ...
    rdr_right_p <- mxAlgebra(h2_RDR_parent * VY, name="rdr_right_p")
    
    rdr_left_o  <- mxAlgebra((2*a_o^2*j + 2*delta_o^2*k) * (2*a_o^2*j + 2*delta_o^2*k + VE_o), name="rdr_left_o")
    rdr_right_o <- mxAlgebra(h2_RDR_offspring * VY, name="rdr_right_o")
