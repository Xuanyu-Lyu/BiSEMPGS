...
fitUniSEMPGS_DisEq_SameTrait_ObservedYPYM <- function(data_path, feaTol = 1e-6, optTol = 1e-8, jitterMean = .5, jitterVar = .1, extraTries = 30, exhaustive = F){
    ...
    ic    <- mxMatrix(type="Full", nrow=1, ncol=1, free=F, values=0, label="ic11", name="ic") 
    
    # gt, ht, itlo, itol are functions of mu and current Omega/Gamma
    ...
    itlo <- mxAlgebra(itlo_Algebra, name="itlo")
    itol <- mxAlgebra(itol_Algebra, name="itol")
    ...
    # thetaNT = a*itlo + a*itol + delta*gt + delta*gt + w
    thetaNT <- mxAlgebra(a*itlo + a*itol + 2*delta*gt + w, name="thetaNT")
    ...
    Params <- list(
                VY, VE, delta, a, k, j, Omega, Gamma, mu, gc, hc, ic, f,
                VY_Algebra, VF_Algebra, Omega_Algebra, Gamma_Algebra, 
                gt_Algebra, ht_Algebra, itlo_Algebra, itol_Algebra, w_Algebra, v_Algebra,
                gt, ht, itlo, itol, w, v,
