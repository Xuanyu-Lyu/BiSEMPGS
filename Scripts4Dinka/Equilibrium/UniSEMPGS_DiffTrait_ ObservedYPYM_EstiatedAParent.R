...
fitUniSEMPGS_DiffTrait_ObservedYPYM_EstimatedAParent <- function(data_path, h2_RDR_offspring, feaTol = 1e-6, optTol = 1e-8, jitterMean = .5, jitterVar = .1, extraTries = 30, exhaustive = F){
    ...
    a_p     <- mxMatrix(type="Full", nrow=1, ncol=1, free=T, values=.8, label="a_p", name="a_p", lbound = .001)     
    
    delta_o <- mxMatrix(type="Full", nrow=1, ncol=1, free=T, values=.4, label="delta_o", name="delta_o") 
    a_o     <- mxMatrix(type="Full", nrow=1, ncol=1, free=T, values=.8, label="a_o", name="a_o", lbound = .001)     
    ...
    Omega_p <- mxMatrix(type="Full", nrow=1, ncol=1, free=T, values=.6, label="Omega_p", name="Omega_p") 
    Gamma_p <- mxMatrix(type="Full", nrow=1, ncol=1, free=T, values=.5, label="Gamma_p", name="Gamma_p") 
    ...
    Params <- list(
        ...
        gt_Algebra, ht_Algebra, rdr_left_o, rdr_right_o, thetaNT, thetaT, Yp_Ym, Yo_Yp, Yp_PGSm,
