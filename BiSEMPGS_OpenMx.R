...
# Create variables and define the algebra for each variables
    ...
    VY    <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(2,.2,.2,1.5), label=c("VY11", "VY12", "VY12","VY22"), name="VY", lbound = -.55) # Phenotypic variance
	VF    <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.5,0,0,.2), label=c("VF11", "VF12", "VF12","VF22"), name="VF", lbound = -.55) # Variance due to VT
    VE    <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.7,.0,0,.5), label=c("VE11", "VE12", "VE12","VE22"), name="VE", lbound = -.55) # Residual variance
...
# Genetic effects:
    ...
    k     <- mxMatrix(type="Symm", nrow=2, ncol=2, free=matrix(c(F,T,T,F),nrow = 2,ncol = 2), values=c(.5,0.02,0.02,.5), label=c("k11", "k12", "k12","k22"),    name="k", lbound = -.55)     # PGS variance (if no AM)
    j     <- mxMatrix(type="Symm", nrow=2, ncol=2, free=matrix(c(F,T,T,F),nrow = 2,ncol = 2), values=c(.5,0,0,.5), label=c("j11", "j12", "j12","j22"),    name="j", lbound = -.55)     # Latent PGS variance (if no AM)
    Omega <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.6,0,0,.5), label=c("Omega11", "Omega21", "Omega12","Omega22"),name="Omega", lbound = -.55) # Within-person PGS-Phen covariance
    Gamma <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.5,0,0,.3), label=c("Gamma11", "Gamma21", "Gamma12","Gamma22"),name="Gamma", lbound = -.55) # Within-person latent PGS-Phen covariance
...
# Assortative mating effects:
    mu    <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.15,0,0,.3), label=c("mu11", "mu21", "mu12","mu22"), name="mu", lbound = -.55) # AM co-path coefficient
    gt     <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.2,0,0,.1), label=c("gt11", "gt21", "gt12","gt22"),  name="gt", lbound = -.55)  # Increase in cross-mate PGS (co)variances from AM
    ht     <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.05,0,0,.05), label=c("ht11", "ht21", "ht12","ht22"),  name="ht", lbound = -.55)  # Increase in cross-mate latent PGS (co)variances from AM
    gc     <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.14,0,0,.1), label=c("gc11", "gc12", "gc12","gc22"),   name="gc", lbound = -.55)  # Increase in within-mate PGS (co)variances from AM
    hc     <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.7,0,0,.25), label=c("hc11", "hc12", "hc12","hc22"),  name="hc", lbound = -.55)  # Increase in within-mate latent PGS (co)variances from AM
    ...
    itlo  <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.05,0,0,.03), label=c("itlo11", "itlo21", "itlo12","itlo22"), name="itlo", lbound = -.55) 
    itol  <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.05,0,0,.03), label=c("itol11", "itol21", "itol12","itol22"), name="itol", lbound = -.55)
    ic   <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.07,0,0,.05), label=c("ic11", "ic12", "ic12","ic22"), name="ic", lbound = -.55) 
