...
# Load the simulated data for this demonstration:
    Example_Data  <- fread("/Users/xuly4739/Library/CloudStorage/OneDrive-UCB-O365/Documents/coding/R-projects/BiSEMPGS/Data/testdata/loop1.txt", header = T)

    cov(Example_Data, use="pairwise.complete.obs")

# Create variables and define the algebra for each variables

    VY    <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(3,0,0,1.5), label=c("VY11", "VY12", "VY12","VY22"), name="VY") # Phenotypic variance
	VF    <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(1.3,0,0,.7), label=c("VF11", "VF12", "VF12","VF22"), name="VF") # Variance due to VT
    VE    <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.7,.0,0,.5), label=c("VE11", "VE12", "VE12","VE22"), name="VE") # Residual variance
...
# Genetic effects:
    ...
    k     <- mxMatrix(type="Symm", nrow=2, ncol=2, free=matrix(c(F,T,T,F),nrow = 2,ncol = 2), values=c(.5,0,0,.5), label=c("k11", "k12", "k12","k22"),    name="k")     # PGS variance (if no AM)
    j     <- mxMatrix(type="Symm", nrow=2, ncol=2, free=matrix(c(F,T,T,F),nrow = 2,ncol = 2), values=c(.5,0,0,.5), label=c("j11", "j12", "j12","j22"),    name="j")     # Latent PGS variance (if no AM)
    Omega <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.6,0,0,.5), label=c("Omega11", "Omega21", "Omega12","Omega22"),name="Omega") # Within-person PGS-Phen covariance
    Gamma <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.5,0,0,.3), label=c("Gamma11", "Gamma21", "Gamma12","Gamma22"),name="Gamma") # Within-person latent PGS-Phen covariance
...
# Assortative mating effects:
    mu    <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.15,0,0,.3), label=c("mu11", "mu21", "mu12","mu22"), name="mu") # AM co-path coefficient
    gt     <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.2,0,0,.1), label=c("gt11", "gt21", "gt12","gt22"),  name="gt")  # Increase in cross-mate PGS (co)variances from AM
    ht     <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.15,0,0,.25), label=c("ht11", "ht21", "ht12","ht22"),  name="ht")  # Increase in cross-mate latent PGS (co)variances from AM
    gc     <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.14,0,0,.1), label=c("gc11", "gc12", "gc12","gc22"),   name="gc")  # Increase in within-mate PGS (co)variances from AM
    hc     <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.7,0,0,.25), label=c("hc11", "hc12", "hc12","hc22"),  name="hc")  # Increase in within-mate latent PGS (co)variances from AM
    ...
    itlo  <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.25,0,0,.3), label=c("itlo11", "itlo21", "itlo12","itlo22"), name="itlo") 
    itol  <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.55,0,0,.13), label=c("itol11", "itol21", "itol12","itol22"), name="itol")
    ic   <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.47,0,0,.35), label=c("ic11", "ic12", "ic12","ic22"), name="ic") 
...
# Vertical transmission effects
    f     <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.4,0,0,.3), label=c("f11", "f21","f12","f22"),  name="f") # Vertical Transmission effect
    w     <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.3,0,0,.51), label=c("w11", "w21", "w12","w22"),  name="w") # Genetic nurture
    v     <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.2,0,0,.54), label=c("v11", "v21", "v12","v22"),  name="v") # Latent nurture
...
# Specify what parameters we're going to be including in our model:
    Params <- list(
                ...
                VY_Constraint, VF_Constraint, Omega_Constraint, Gamma_Constraint, 
                #gt_constraint, 
                ht_constraint, gc_constraint, hc_constraint, gchc_constraint, 
                itlo_constraint, itol_constraint, ic_constraint, v_constraint, 
...
# Create the model:
    ...
    fitModel1 <- mxTryHard(Model1, extraTries = 50, intervals=T, silent=F)
