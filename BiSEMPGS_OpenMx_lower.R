...
# Create variables and define the algebra for each variables
    ...
    VY    <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(3,0,0,1), label=c("VY11", "VY12", "VY12","VY22"), name="VY") # Phenotypic variance
...
# Assortative mating effects:
    mu    <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.15,0,0,.3), label=c("mu11", "mu12", "mu12","mu22"), name="mu") # AM co-path coefficient
    ...
    ht     <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.15,0,0,.5), label=c("ht11", "ht21", "ht12","ht22"),  name="ht")  # Increase in cross-mate latent PGS (co)variances from AM
    gc     <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.4,0,0,.1), label=c("gc11", "gc12", "gc12","gc22"),   name="gc")  # Increase in within-mate PGS (co)variances from AM
    hc     <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.7,0,0,.5), label=c("hc11", "hc12", "hc12","hc22"),  name="hc")  # Increase in within-mate latent PGS (co)variances from AM
...
# Vertical transmission effects
    f     <- mxMatrix(type="Symm", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.4,0,0,.3), label=c("f11", "f12","f12","f22"),  name="f") # Vertical Transmission effect
    w     <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(.3,0,0,.51), label=c("w11", "w12", "w21","w22"),  name="w") # Genetic nurture
    v     <- mxMatrix(type="Full", nrow=2, ncol=2, free=c(T,T,T,T), values=c(2,0,0,.54), label=c("v11", "v12", "v21","v22"),  name="v") # Latent nurture
...
# Create the model:
    ...
    fitModel1 <- mxTryHard(Model1, intervals=F, silent=F)
